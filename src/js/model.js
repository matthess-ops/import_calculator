export default class Model {
  constructor() {
    ...
    // this is dummy data, its get overwritten/ usefull for debugging
    // these are input values
    this.countries = [
      { alpha3Code: "NLD", nameEnglish: "Yemen", nameDutch: "Jemen" },
      { alpha3Code: "GLD", nameEnglish: "Overijssel", nameDutch: "Rijssen" },
    ];
    this.currencies = [
      {
        primaryCurrencyCode: "YER",
    ...
    this.shipmentServices = [
      { costs: "10.50", shipper: "UPS" },
      { costs: "13.0", shipper: "POSTNL" },
      { costs: "10.50", shipper: "FEDEX" },
      { costs: "10.00", shipper: "CUSTOM" },
    ];
    //end constant values
  ...
  bindInputUIFieldsChange(callback) {
    this.onInputUIFieldsChange = callback;
  }

  //binder to bind output values
  bindOutputUIFieldsChange(callback) {
  ...
  _commit(whichFunction) {
    this.onInputUIFieldsChange(
      this.countries,
      this.currencies,
      this.importDutyPercentage,
      this.taxPercentage,
      this.clearanceCosts,
      this.shipmentServices,
      this.exchangeRate,
  ...
  _calculateOutputValues() {
  
    const conversionToEU = 1.0 / parseFloat(this.exchangeRate);
    this.totalOrderAmount =
      this._roundToTwo((parseFloat(this.orderAmount) + parseFloat(this.shippingCosts)) *
      conversionToEU);

    // it total order cost is below 22 euros no importduty and no tax
    // but if the order is from outside the EU clearance costs are included 
    if (this.totalOrderAmount < 22) { 
      this.importDutyAmount = 0;
      this.taxAmount = 0;

      // if tax percentage is 0 this means that the packege comes from a
      ...
      } else { // package from outside the eu clearance costs are inclued
        this.totalAdditionalAmount =this._roundToTwo(
          this.importDutyAmount +
          this.taxAmount +
          parseFloat(this.clearanceCosts));
      }
    ...
    if (this.totalOrderAmount > 22 && this.totalOrderAmount < 150) {
      this.importDutyAmount = 0;
      this.taxAmount =
      this._roundToTwo((this.totalOrderAmount / 100) * parseFloat(this.taxPercentage));
           // if tax percentage is 0 this means that the packege comes from a
      // eu country. Thus no clearance costs are included.
    ...
    if (this.totalOrderAmount > 150) {
      this.importDutyAmount =
      this._roundToTwo((this.totalOrderAmount / 100) * parseFloat(this.importDutyPercentage));
      ...
      if (this.taxPercentage == 0) {
        ...
        this.totalAdditionalAmount =
        this._roundToTwo(this.importDutyAmount +
          this.taxAmount +
          parseFloat(this.clearanceCosts));
      }
  ...
  _updateCurrencySymbolByCurrencyCode(currencyCode) {
    try {
      Object.keys(this.countriesData).forEach((key) => {
        if (this.countriesData[key]["primaryCurrencyCode"] == currencyCode) {
          this.symbol = this.countriesData[key]["primaryCurrencySymbol"];
        }
      });
    } catch (err) {
      this.symbol = "";
  ...
  _addCountryToLastUsedValues() {
    ...
    this.countriesLastUsedValues[this.countries[0].alpha3Code] = newValues;
    // could be cleaner by making setting and stringifying a seperate function
    //thus preventing double code
    localStorage.setItem(
      "countriesLastUsedValues",
      JSON.stringify(this.countriesLastUsedValues)
    );
    localStorage.setItem(
      "lastCountryCode",
      JSON.stringify(this.lastCountryCode)
    );
  ...
  changeCurrenyName(currencyCode) {
  ...
  //controller triggered
  changeOrderAmount(orderAmount) {
    //maybe do input validation here if text =  number is not sufficient
    this.orderAmount = orderAmount;
  }
//controller triggered
  changeShippingCosts(shippingCosts) {
    //maybe do input validation here if text =  number is not sufficient
    this.shippingCosts = shippingCosts;
  }
//controller triggered
  changeImportDutyPercentage(importDutyPercentage) {
    this.importDutyPercentage = importDutyPercentage;
  }
//controller triggered
  changeTaxPercentage(taxPercentage) {
    this.taxPercentage = taxPercentage;
  }
//controller triggered
  changeClearanceCosts(clearanceCosts) {
...
changeShipmentService(shipperInput) {
    let shipperElement = null; 
    const newClearanceCosts = this.shipmentServices.find((shipmentService) => {
    ...
    let copyShipmentServices = this.shipmentServices;
    //the order of shipment Services in the shipment Services dropdown/select is reorderd so that
    //the selected shipment that has been selected is now the top service.
    copyShipmentServices = copyShipmentServices.filter(
      (shipper) => shipper.shipper !== shipperInput
    );
  ...
  //therefore 
  changeCountryName(inAlpha3Code) {
    ...
    if (this.countriesLastUsedValues[inAlpha3Code] === undefined) {
      ...
      const countryCurrencyCode = this.countriesData[inAlpha3Code][
        "primaryCurrencyCode"
      ];

      this._updateCurrenciesForCurrencySelect([
        countryCurrencyCode,
        "USD",
        "EUR",
      ]);

      this.exchangeRate = this.exchangeRates[countryCurrencyCode][
        "exchangeRate"
      ];
      this._updateCurrencySymbolByCurrencyCode(countryCurrencyCode);

      this.importDutyPercentage = this.countriesData[
        inAlpha3Code
      ].importDutyRate;
      ...
      this._updateCountriesForCountrySelect([inAlpha3Code]);
      const countryCurrencyCode = this.countriesLastUsedValues[inAlpha3Code]
        .currencyCode;
      ...
      this.importDutyPercentage = this.countriesLastUsedValues[
        inAlpha3Code
      ].importDuties;
      this.taxPercentage = this.countriesLastUsedValues[inAlpha3Code].taxRate;
      this.changeShipmentService(
        this.countriesLastUsedValues[inAlpha3Code].shipmentService
      );
      //first do changeShipmentService because that writes the clearancecost associated with the
      ...
      this.clearanceCosts = this.countriesLastUsedValues[
        inAlpha3Code
      ].clearanceCosts;
  ...
  _updateCurrenciesForCurrencySelect(currenciesArray) {
    const importantCurrenciesArray = currenciesArray.reverse(); // reverse array

    let sortedCurrenciesKeys = Object.keys(this.exchangeRates).sort();

    importantCurrenciesArray.forEach((currencyCode) => {
      sortedCurrenciesKeys = sortedCurrenciesKeys.filter(
        (e) => e !== currencyCode
      ); // remove countrycode
    ...
    sortedCurrenciesKeys.forEach((element) => {
      let nameEnglish = "dummy";
      let curSymbol = "dummy";

      Object.keys(this.countriesData).forEach((country) => {
        if (
          this.countriesData[country]["primaryCurrencyCode"] ==
          this.exchangeRates[element].currencyCode
        ) {
          nameEnglish = this.countriesData[country]["primaryCurrencyName"];
          curSymbol = this.countriesData[country]["primaryCurrencySymbol"];
        }
      });

      const newElement = {
        primaryCurrencyCode: this.exchangeRates[element].currencyCode,
        primaryCurrencyName: nameEnglish,
        primaryCurrencyNameDutch: this.exchangeRates[element].currencyNameDutch,
  ...
  _updateCountriesForCountrySelect(countriesArray) {
    let copiedCountriesData = this.countriesData;
    const importantCountriesArray = countriesArray.reverse(); // reverse array
    ...
    //should be first
    let sortedCountryKeys = Object.keys(copiedCountriesData).sort();

    importantCountriesArray.forEach((countryCode) => {
      sortedCountryKeys = sortedCountryKeys.filter((e) => e !== countryCode); // remove countrycode
      sortedCountryKeys.splice(0, 0, countryCode); // place countrycode as first in array
    });

    let sortedCountries = [];
    sortedCountryKeys.forEach((element) => {
      const newElement = {
        alpha3Code: this.countriesData[element].alpha3Code,
        nameEnglish: this.countriesData[element].nameEnglish,
        nameDutch: this.countriesData[element].nameDutch,
      };
  ...
  _loadStartUIValues() {
    if (this._checkIfUserHasLocalData() == false) {
      ...
      const lastUSedValues = this.countriesLastUsedValues[this.lastCountryCode];

      this._updateCountriesForCountrySelect([
      ...
      this.exchangeRate = this.exchangeRates[lastUSedValues.currencyCode][
        "exchangeRate"
      ];
      this._updateCurrencySymbolByCurrencyCode(lastUSedValues.currencyCode);
      this._updateCurrenciesForCurrencySelect([
        lastUSedValues.currencyCode,
        "USD",
        "EUR",
      ]);
  ...
  writeExchangeRatesToLocal() {
    localStorage.setItem(
      "exchangeRates",
      JSON.stringify(this.downloadedExchangeRates)
    );
  }

  writeCountriesDataToLocal() {
    localStorage.setItem(
      "countryData",
      JSON.stringify(this.downloadedCountryData)
    );
  }

  readExchangeRatesToLocal() {
    return JSON.parse(localStorage.getItem("exchangeRates")) || [];
  }

  readCountriesDataToLocal() {
  ...
  createReadCountriesLastUsedValues() {
